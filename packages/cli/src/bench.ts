...
/**
 * スカラ鎖のシナリオの列数。**ブック全体の平均依存度を要件 N-1 の 3 に寄せるために選んだ値。**
 *
 * **要件が言う「平均依存度 3」はブック全体の値である。** 行の先頭 3 列は種となる定数で
 * 依存を持たないので、平均は `3(W-3)/W` になり、**W をいくら大きくしても 3 には届かない**
 * （種が 1 つでもある限り構造的にそうなる）。**近づけるほど鎖が長くなる**ので、
 * 1 セル変更の下流が肥大して「増分」を測らなくなる。
 *
 * | 列数 | ブック全体の平均依存度 | 1 セル変更の下流 |
 * | --- | --- | --- |
 * | 10 | 2.10 | 7 件 |
 * | **100** | **2.91** | **97 件** |
 * | 1000 | 2.99 | 997 件 |
 *
 * **100 列を採った。** 要件の 3 に対して 3% 落ちで、下流は 100 件に収まる。
 */
const CHAIN_COLUMNS = 100;

/** スカラ鎖で定数にする列の数。**数式セルはこの数だけ左を読む**ので、依存度に等しい。 */
 ...
 * 数式セルはどれもちょうど 3 つのセルに依存し、**ブック全体の平均依存度は 2.91**
 * （種となる定数セルがあるので 3 には届かない。`CHAIN_COLUMNS` にその理由がある）。
 *
...
export function scalarChain(cells: number): Scenario {
  ...
  return {
    name: 'スカラ鎖',
    description: `${CHAIN_COLUMNS} 列を 1 行とし、左 ${CHAIN_DEGREE} 列は定数、残りは左 ${CHAIN_DEGREE} つの和（ブック全体の平均依存度 2.91、要件 N-1 は 3）`,
    contents,
    // 行の先頭。**この行の数式セルすべての上流**にあたる。
    edit: { address: cellAt(1, 1), contents: ['1', '2'] },
  };
}
