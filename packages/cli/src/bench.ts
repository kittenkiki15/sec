...
/** スカラ鎖のシナリオの列数。**10 列で 1 行**にすると、10,000 セルが 1,000 行になる。 */
const CHAIN_COLUMNS = 10;

/** スカラ鎖で定数にする列の数。**数式セルはこの数だけ左を読む**ので、平均依存度に等しい。 */
 ...
 *
 * **10 列を 1 行とし、左の 3 列を定数、残りを「左 3 つのセルの和」にする。**
 * 数式セルはどれもちょうど 3 つのセルに依存するので、**平均依存度が厳密に 3** になる。
 *
 * **依存が行の中に閉じている。** 1 セル変更したときの下流が同じ行の 7 セルに限られ、
...
export function scalarChain(cells: number): Scenario {
  ...
  return {
    name: 'スカラ鎖',
    description: `${CHAIN_COLUMNS} 列を 1 行とし、左 ${CHAIN_DEGREE} 列は定数、残りは左 ${CHAIN_DEGREE} つの和（平均依存度 ${CHAIN_DEGREE}）`,
    contents,
